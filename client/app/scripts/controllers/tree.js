 ...
 */
 ...
 .controller('TreeCtrl', ['$http', function ($http) {
 	var req = $http.get('/api/tree');
    var scope = this;
    ...
    req.catch(function (err) {
 	...
 	function buildFamilyTree(treeData) {
 		...
 		var getChildren = function(d){
 			...
 			if(d.kids) for(var i = 0; i < d.kids.length; i++){
 				d.kids[i].isRight = true;
 				d.kids[i].parent = d;
 				a.push(d.kids[i]);
 			}
 		...
 		var vis = d3.select("#familyTree").append("svg")
 		...
 		t2 = d3.layout.tree().size([height, halfWidth]).children(function(d){return d.kids;});
 		t1.nodes(root);
 		t2.nodes(root);
 		...
 		function update(source) {
	        ...
	        nodes.forEach(function(d) { d.y = d.depth * 230 + halfWidth; });
	        ...
	        var nodeUpdate = node.transition().duration(duration).attr("transform", function(d) { var p = calcLeft(d); return "translate(" + p.y + "," + p.x + ")"; });
